...
struct Camera {
  ...
  ray_dy      : vec4<f32>,
  prev_ray00  : vec4<f32>,
  prev_ray_dx : vec4<f32>,
  prev_ray_dy : vec4<f32>,
...
fn ray_dir_from_pixel_prev(px: vec2<f32>) -> vec3<f32> {
  let d = cam.prev_ray00.xyz + px.x * cam.prev_ray_dx.xyz + px.y * cam.prev_ray_dy.xyz;
  return normalize(d);
}

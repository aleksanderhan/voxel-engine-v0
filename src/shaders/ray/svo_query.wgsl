...
// Returns leaf material at world position by converting to camera-relative space.
fn query_leaf_world(p: vec3<f32>) -> LeafQuery {
  let vs = cam.voxel_params.x;
  let chunk_size_m = f32(cam.chunk_size) * vs;

  // Convert local meters -> local chunk coords
  let p_local = world_to_local(p);
  let c = chunk_coord_from_pos(p_local, chunk_size_m);
  ...
  if (slot == INVALID_U32 || slot >= cam.chunk_count) {
    ...
    return make_air_leaf(p_local, vs);
  }
  ...
  return query_leaf_at(p_local, root_bmin, root_size, ch.node_base, ch.macro_base);
}

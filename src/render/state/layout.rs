...
pub fn create_layouts(device: &wgpu::Device) -> Layouts {
    ...
    // 15 primary hit payload history (storage read/write)
    // 16 sun-shadow history (sampled)
    // 17 sun-shadow history output (storage buffer write)

...
impl<'a> ChunkManagerQuery<'a> {
    ...
    fn solid_from_col_u16(v: u16, ly: i32) -> bool {
        let lo = (v & 0x00FF) as i32;
        let hi = ((v >> 8) & 0x00FF) as i32;

        // common sentinel patterns (safe-ish):
        if v == 0xFFFF {
            return false;
        }

        // If it looks like a (min,max) inclusive range packed into bytes, use it.
        // Works well for CHUNK_SIZE=64: values are 0..63.
        let looks_like_range = (0..64).contains(&lo) && (0..64).contains(&hi) && hi >= lo;
        if looks_like_range {
            return ly >= lo && ly <= hi;
        }

        // Otherwise interpret as "surface height" in low byte.
        // Terrain convention: solid below or at surface.
        if (0..64).contains(&lo) {
            return ly <= lo;
        }
